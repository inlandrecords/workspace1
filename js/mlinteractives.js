	...
	ml.setUpEvaluator = function(calcID, evalObject) {
		for (var graphID in evalObject) {
			calcID.setExpressions([
				{id: evalObject[graphID].segs[0], color: ml.COLORS.RED},
				{id: evalObject[graphID].segs[1], color: ml.COLORS.LIGHTORANGE},
				{id: evalObject[graphID].pt, color: ml.COLORS.ORANGE}
			]);
		}
	};
